	...
	if(!empty($code)) {
		...
		if(!is_error($ret)) {
			...
			if(is_array($auth) && !empty($auth['openid'])) {
				$_SESSION['oauth_openid'] = $auth['openid'];
				...
				if ($scope == 'userinfo') {
					...
					if (!is_error($response)) {
						...
						// Add by kaisa: fix missing avatar, 132 is weixin's spec
						$userinfo['avatar'] = rtrim($userinfo['headimgurl'], '0') . 132;
						// Add By kaisa: Session store userinfo for userinfo
