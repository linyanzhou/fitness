...
if ($operation == 'display') {
	...
	$sql = "select  * from " . tablename('tg_order') . " where $condition   ORDER BY createtime DESC " . "LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
	...
	$total_tuan = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_tuan=1", $paras);
	$total_single = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_tuan=0", $paras);
	$total_all = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2");
 ...
 //导入CSV 
    ...
    for ($i = 1; $i < $len_result; $i++) { //循环获取各字段值 
		...
		if (!empty($expressOrder) && !empty($expressName)) {
			$res = pdo_update('tg_order', array('status' => 3, 'express' => $expressName, 'expresssn' => $expressOrder), array('orderno' => $orderNo));
			if ($res) {
				$succ_result += 1;
			} else {
				$error_result += 1;
...
//	$orders = pdo_fetchall("select * from" . tablename('tg_order') . "where $condition and status=2");
	$orders = pdo_fetchall("select DISTINCT address from" . tablename('tg_order') . "where $condition and status=2");
	if ($status == '0') {
		$str = '单独购买待发货订单_' . time();
	}
	if ($status == '1') {
	...
	foreach ($orders as $k => $vv) {
		$order_same = pdo_fetchall("select * from" . tablename('tg_order') . "where address='{$vv['address']}' and status=2 and $condition");
