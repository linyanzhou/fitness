...
class Message {
	...
	public static function group_success($tuan_id, $the, $url1, $url2) {
		...
		$content = "组团成功!!!";
		$alltuan = pdo_fetchall("select * from" . tablename('tg_order') . "where tuan_id = '{$tuan_id}' and status in(1,2,3,4)");
		foreach ($alltuan as $num => $all) {
			$order = pdo_fetch("select * from" . tablename('tg_order') . "where orderno = '{$all['orderno']}'");
			$goods = pdo_fetch("select * from" . tablename('tg_goods') . "where id = '{$order['g_id']}'");
			$tuan_first_order = pdo_fetch("select * from" . tablename('tg_order') . "where tuan_first=1 and tuan_id='{$tuan_id}'");
