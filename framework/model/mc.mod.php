...
function mc_oauth_fans($openid, $acid){
	$sql = 'SELECT `openid`, `uid` FROM '.tablename('mc_oauth_fans').' WHERE oauth_openid=:openid ORDER BY id DESC';
...
function mc_oauth_userinfo($acid = 0) {
	...
	if (isset($_SESSION['userinfo'])) {
		...
		// Change by kaisa: 不考虑没有subscribe的情况, OAuth已获取信息
		// if (!empty($userinfo['subscribe']) || !empty($userinfo['nickname'])) {
		if (!empty($userinfo['nickname'])) {
			return $userinfo;
		} else {
			error_log("Fail to get userinfo from SESSION");
		}
	}
	if ($_W['container'] != 'wechat') {
		...
		// Add by kaisa: 只有订阅用户，才可获取其nickname哦（所以应该在OAuth时获取用户数据
	...
	// Add by kaisa: support uni oauth,temp hack for proxy_redirect host
	if (strstr($unisetting['oauth']['host'], 'proxy_redirect')) {
	  $proxy_host = str_replace('proxy_redirect.', '', $unisetting['oauth']['host']);
	  $app_url =  $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}{$str}&c=auth&a=oauth&scope=userinfo";
	  $proxy_url = $proxy_host . "?platform=wechat&proxy_redirect_base64=" . base64_encode($app_url);
